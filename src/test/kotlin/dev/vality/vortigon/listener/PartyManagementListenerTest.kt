...
import dev.vality.vortigon.service.DomainRepositoryAdapter
...
class PartyManagementListenerTest : AbstractKafkaIntegrationTest() {
    ...
    fun `test party suspension event handle`() {
        ...
        assertEquals(partyId, party?.partyId)
        assertEquals(sequenceId.get(), party?.eventId)
        assertTrue(party?.suspension == dev.vality.vortigon.domain.db.enums.Suspension.active)
        assertEquals(
    ...
    fun `test contract create event handle`() {
        ...
        assertTrue(contract?.status == dev.vality.vortigon.domain.db.enums.ContractStatus.active)
    }
    ...
    fun `test shop suspension handle`() {
        ...
        assertEquals(shopId, shop?.shopId)
        assertEquals(sequenceId.get(), shop?.eventId)
        assertTrue(shop?.suspension == dev.vality.vortigon.domain.db.enums.Suspension.suspended)
        assertEquals(
